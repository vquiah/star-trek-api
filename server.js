...
app.get('/api/:alienName', (req, res)=>{
      const alientName  =req.params.alienName.toLocaleLowerCase()
      if(aliens[alientName]){  //listen to the req to see if we have it
          res.json(aliens[alientName])
      }else{
          res.json(aliens['human'])
      }
})

app.listen(process.env.PORT || PORT, ()=>{
    console.log(`Server is running on port ${PORT}, go and catch it!`)
})
